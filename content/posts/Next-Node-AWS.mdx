...
You already know how to set up a Next.js, Node.js and Express.

_This is NOT a walkthrough of creation of a Next.js and Node.js app; though you can look more online for a tutorial if you need._

Preferred, but not essential, familair with AWS, EC2 and Docker. Will delve into each, but nor is this a detailed deep look into them.
...
You may wish to go the route of a static generated build, for this demo, have gonne for SSR.
...
I like to have two Docker files, one for local builds as one for production, which I neame **Dockerfile.prod**.
...
services:
  nextjs-app:
    build:
      context: ./client
      dockerfile: Dockerfile // or Dockerfile.prod if tyou named that
    ports:
      - "3000:3000"
    environment:
...
You will need this not only for the env vars but for the deployment futher down. Access you instance on AWS EC2 and look for the public IPV4 url, (for example 54.123.12.11) and make a copy. You can also try: this in step 1 instead: **EC2_PUBLIC_IP=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)**
...
**NOTE - You may run into SSH authentication isues, if you do you may need to create a SSH key in your GitHUb.**
